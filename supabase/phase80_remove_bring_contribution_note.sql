-- Circles phase 80: remove the unused note field from food contributions

begin;

-- The event-cloning function previously copied the note column.
-- Replace it first so the column can be removed safely.
...
begin
  ...
  insert into public.event_bring_contributions (
    event_id, need_id, user_id, item_name, quantity, created_at, updated_at
  )
  select
    target_event_id,
    target_need.id,
