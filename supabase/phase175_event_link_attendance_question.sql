-- Circles phase 175
-- Keep the event-attendance question available throughout the event's calendar day
-- in Israel, including when the user is not yet a circle member.
...
begin
  ...
  if target_event.status = 'cancelled'
    or (target_event.starts_at at time zone 'Asia/Jerusalem')::date
    ...
    if pending_request.requested_event_id is not null
      ...
      and exists (
        ...
        where event.id = pending_request.requested_event_id
          ...
          and (event.starts_at at time zone 'Asia/Jerusalem')::date
